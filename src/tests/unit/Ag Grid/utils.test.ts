...
describe("Testing getNoOverlayNoRowsTemplate", () => {
    it("It should return correct message", () => {
        ...
        expect(getNoOverlayNoRowsTemplate({ entity: "stocks" })).toBe(
            getResult(`There are no stocks available.`),
        );
        const customMessage = "Something went wrong";
        expect(getNoOverlayNoRowsTemplate({ customMessage: customMessage })).toBe(getResult(customMessage));
    });
});
