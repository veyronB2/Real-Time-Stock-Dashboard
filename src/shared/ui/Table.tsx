...
import { getMenuItems } from '../grid/utils';
...
const Table = <T,>({
    ...
    const defaultGridStyle = useMemo(
        (): CSSProperties => ({
            height: '600px',
            minHeight: '18.75rem',
            width: '100%',
    ...
    const mergedGridStyles = useMemo(
        (): CSSProperties => ({
            ...defaultGridStyle,
            ...gridStyle,
        }),
    ...
    const mergedGridOptions = useMemo((): AgGridReactProps => {
        ...
        updatedOptions.getContextMenuItems = () => getMenuItems(contextMenuItems, 'context');
        updatedOptions.getMainMenuItems = () => getMenuItems(mainMenuItems, 'main');
